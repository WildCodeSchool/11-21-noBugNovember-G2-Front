...
const Disconnect = ({setAvatar, setIsConnected, admin, setAdmin}) => {
  const [updateAvatar, setUpdateAvatar] = useState("");
	...
	const deleteArticle = (params) => {
		axios
      .delete('http://localhost:3030/articles/delete',
      {data: {id: params}})
  }
	...
	const getMyArticles = () => {
		axios.post("http://localhost:3030/articles/byuser",
			{
	...
	const getAllArticles = () => {
		axios.get("http://localhost:3030/articles/godmode/read")
			.then(response => response.data)
  ...
  return (
    <div className="Disconnect">
      ...
      <input type="button" id='submit' onClick={() => aurevoir()} value='Se déconnecter'></input>
			...
			<h2>Gestion des articles postés en veille - Mode Admin</h2>
				...
				<input type="button" id="avatar" onClick={() => getAllArticles()} value="Voir les articles postés"></input>
					<table className={reveal?"reveal":"cache"}>
						...
						{bdd.slice(0, moreArticle).map(detail => (
							<tr>
								...
								<th><i onClick={() => deleteArticle(detail.id)} class="fas fa-times-circle"></i></th>
							</tr>
						))}
						</tbody>
					</table>
			...
			<div className={!admin?"perso":"cache"}>
				...
				<h3>⚠ Toutes suppresion est définitive !</h3>
				<div className="linearticle">
				<input type="button" id="avatar" onClick={() => getMyArticles()} value="Voir vos articles postés"></input>
					<table className={reveal?"reveal":"cache"}>
						<thead className={reveal?"reveal":"cache"}>
							<tr className={reveal?"reveal":"cache"}>
